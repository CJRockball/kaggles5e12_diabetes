...
def best_params():
    param_test = {   # Learning control - CRITICAL for good performance
    'learning_rate': 0.02,           # Lower than default 0.1 for better generalization
    'num_leaves': 31,                # Conservative, prevents overfitting
    'max_depth': -1,                 # Let num_leaves control complexity
    
    # Regularization - KEY for preventing overfitting
    'min_child_samples': 20,         # Min 20 samples per leaf (default is good)
    'min_child_weight': 0.001,       # Minimum hessian sum in leaf
    'reg_alpha': 0.1,                # L1 regularization
    'reg_lambda': 0.1,               # L2 regularization
    'min_split_gain': 0.0,           # Minimum gain to split
    
    # Sampling - reduces overfitting and speeds training
    'subsample': 0.8,                # Row sampling (bagging)
    'bagging_freq': 5,               # Bagging every 5 iterations
    'colsample_bytree': 0.8,         # Feature sampling per tree
    
    # Speed and stability
    'max_bin': 4096,                  # Default, good for most cases
    "feature_pre_filter": False
    }
    return param_test
...
for i,(train_index, valid_index) in enumerate(kf.split(df_X)):
    ...
    # LGBM
    # PREP LGBM MODEL
    trn_data = lgb.Dataset(Xtrain_te, ytrain)
    lgb_eval = lgb.Dataset(Xvalid_te, yvalid)
    
    new_params = best_params()
    # TRAIN MODEL, PREDICT Y
    model = lgb.train(params | new_params, trn_data, NUM_ROUNDS,
                    valid_sets=lgb_eval, 
                    callbacks=[
    ...
    ypred_proba = model.predict(Xvalid_te) 
    ...
    preds += model.predict(Xtest_te) / KFOLD
